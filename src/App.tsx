...
function App() {
  ...
  const handleAdd = (id: number, index: number) => {
    ...
    if (parentSelect) {
      ...
      selectedIngredients.splice(index , 0, { id: newSelect.id, label: "-", value: "0" })
          ...
          {selects.map((select, index) => (
            <div key={index}>
              <Select
                ...
                onRemove={select.id > 6 ? () => handleRemove(select.id) : undefined} // Удаляем кнопку удаления для первого компонента
